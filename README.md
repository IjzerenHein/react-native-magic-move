...
| `parentScaleHint` | `number | {x: number, y: number}` |                             | Optional scale-hint. Use this prop when your component is scaled using a transform somewhere higher up render-tree. This will tell the component how it is scaled so it can be rendered correctly. |
